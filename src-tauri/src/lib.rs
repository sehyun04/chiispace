...
fn pty_open(
    ...
    let session = PtySession::start(PtyOptions {
        ...
        cwd,
        shell,
        // claude 에게 대체 화면을 쓰지 말라고 한다. 그래야 화면을 고쳐 그릴 때마다
        // 그 내용이 터미널 스크롤백에 쌓여서, 지난 대화를 휠로 되짚어 볼 수 있다.
        // 대체 화면을 쓰면 스크롤이 claude 안에서만 일어나 위로 올려도 아무것도 없다.
        //
        // **엔진 기본값이 아니다.** kasaterm 은 자기 렌더러에 「맨 위 질문 고정」 띠가
        // 따로 있어서 스크롤백 대신 화면 안정성(안 깜빡이고 입력창이 제자리)을 골랐고,
        // 그 결정이 rev 를 올릴 때 따라온다. 우리에겐 그 띠가 없어 터미널 스크롤백이
        // 되짚어 보는 유일한 길이라 여기서 되돌린다. 엔진 rev 를 올린 뒤 "위로 스크롤이
