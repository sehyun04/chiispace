...
- Codex는 로컬 CLI 그대로 실행한다. 자체 App Server·원격 TUI 중계·대화 ID별 잠금은 제거했다.
  ...
  원문 프롬프트·첨부·임의 MCP 인증은 복제하지 않는다. 외부 원격 연결 등 실행 정보를 모르는 칸은 수동 선택한다.
  새로운 복원 실패는 명령·옵션을 보존하고 자동 재시도만 멈춘다.
...
CLI가 보내는 터미널 타이틀을 쓴다. 자동 이름도 `paneTitles`로 별도 저장해 다음 시작부터
헤더·옆 목록·칸 연결 도구에 바로 표시한다. 직접 붙인 이름이 우선이며, 새 CLI 제목을 받으면
자동 이름만 갱신한다. 셸 경로·기본 프로그램 이름·일시적인 실행 명령은 저장된 이름을 덮지 않는다.
숨긴 탭도 저장하고 닫힌 칸은 저장 대상에서 뺀다. 제목 안의 `/`는 경로처럼 잘라 버리지 않는다.
저장한 이름이나 새 제목이 전혀 없을 때만 프로세스 이름을 표시한다.
이것은 칸의 마지막 표시 이름이지 continue가 원래 대화를 골랐다는 보장이 아니다.
...
작업 폴더별 최근 본문, 재시작, `/new` 뒤 최근 대화, 숨겨진 탭, 같은 폴더의 중복 칸 선택 목록을 검증한다.
권한 포함 프로필과 명시적인 CLI 권한은 이전 본문·모델 입력과 실제 turn 기록으로 확인한다.
문법 오류 프로필은 명령·옵션을 보존하고 자동 재시도를 멈추는지 검증한다.
`CHIISPACE_TEST_CODEX_PATH=inherited`로 npm/Node 경로도 검증하며 사용자 세션·설정 해시를 비교한다.

Claude는 `CHIISPACE_TEST_REAL_CLAUDE`에 설치된 exe를 지정한 뒤
`node --test scripts/claude-continue.test.mjs`를 실행한다. 별도 `CLAUDE_CONFIG_DIR`에 합성 대화를 만들고
구형 ID 복원 명령이 `--continue`로 바뀌어 실제 CLI에 이전 내용이 표시되는지 확인한다.
테스트 프로젝트만 신뢰하고 가짜 인증 키·루프백 서버를 사용한다. CLI 연결 확인 외 모델 API 요청은 허용하지 않는다.

백스페이스는 `node --test scripts/terminal-input.test.mjs`로 실제 WebView의 textarea에
`KeyboardEvent`·조합·blur 이벤트를 보내고 실제 PTY 수신 바이트를 확인한다.
...
- `PROBE` 는 JS 표현식의 결과를 화면 아래 오버레이에 찍는다. 릴리스 웹뷰에는 콘솔이 없어서
  스크린샷이나 격리 테스트의 로컬 수집기로 결과를 받는다.
