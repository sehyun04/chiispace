...
옆칸의 **새 탭**을 누르면 어느 셸로 열지 먼저 묻는다 — 명령 프롬프트 · Windows
PowerShell · PowerShell 7 · Git Bash 중 **이 컴퓨터에 실제로 있는 것만** 뜬다. 셸은
탭에 붙으므로 그 탭 안에서 칸을 나누면 다 같은 셸이고, 다른 셸이 필요하면 탭을 새로
연다. 고른 셸은 탭 제목 옆에 작게 남고 세션에도 저장돼 다시 켜도 그대로다.
단축키(`Ctrl+Shift+T`)는 묻지 않고 지금 탭과 같은 셸로 연다.

pane 헤더를 **두 번 누르면 이름을 붙인다**. 자동으로 보여 주는 것(돌고 있는 명령,
claude 대화의 마지막 프롬프트)은 추측이라, 직접 붙인 이름이 있으면 그것이 이긴다.
비우면 다시 자동 표시로 돌아간다.

그 pane 이 claude 를 돌리고 있으면 **대화 이름도 같이 바뀐다**(`/rename`). 그래서 나중에
`claude --resume` 목록에서도 같은 이름으로 보인다 — pane 이름과 대화 이름이 따로 놀면
정작 세션을 고를 때 아무 도움이 안 된다.
...
- **pane 분할** — 좌우 · 상하, 경계선 드래그, 방향키로 포커스 이동. 칸을 늘리면 전체가
  고르게 다시 나뉜다(한 자리만 계속 반씩 줄어들지 않게)
- **pane 재배치** — 헤더를 끌어 옮긴다. 가운데는 맞바꾸기, 가장자리는 그쪽으로 갈라 붙이기.
  배치 트리를 다시 엮어도 PTY 는 안 죽는다 — slot 이 배치 모양이 아니라 id 로 짝지어져서다
- **탭** — 탭마다 자기 배치와 자기 폴더. 탭 줄을 위에 따로 두지 않고 사이드바 목록에
  묶음으로 넣었다 — 같은 것을 두 군데서 고르게 하지 않으려고
- **세션 목록** — 사이드바에 탭이 묶음으로, 그 아래 각 pane 이 아이콘·이름·폴더로 뜬다.
  어느 칸에서 무엇이 돌고 있는지가 한눈에 보이고, 누르면 그 탭으로 건너가 그 pane 을 잡는다.
  파일 탐색기는 두지 않는다 — 파일을 뒤지는 일은 pane 안의 에이전트가 한다
- **git** — 브랜치 · ahead/behind · 바뀐 파일 수
- **pane 헤더** — 실행 중인 명령 이름, 에이전트 표시, 작업 중 바
- **세션 복원** — 탭 · 배치 · 폴더 · 글자 크기, 그리고 그 pane 이 돌리던 것.
  claude 를 켜 둔 pane 은 다시 켤 때 그 대화가 저절로 다시 열린다. pane 마다 어느
  대화였는지를 세션 ID 로 붙여 두므로, pane 이 여럿이면 각자 자기 대화로 돌아온다.
  pane 헤더에는 그 대화에서 마지막으로 시킨 일이 걸린다 — 여러 개를 띄워 놓고 어느 쪽이
  무슨 작업이었는지 가리려면 그게 있어야 한다 — 이어 여는 것은 대화를 불러오는 것뿐이라 부작용이 없다. 빌드·배포
  같은 명령은 실행하지 않고 프롬프트에 쳐 두기만 한다. 저 혼자 다시 도는 건 곤란하다
- **캐릭터 배정** — 칸마다 그림이 들어온 사람 중 한 명이 맡는다. 아직 얼굴이 없는 사람은
  세우지 않는다 — 스무 명을 다 세우면 색 동그라미가 섞여 나와 들어온 그림이 묻힌다.
  그림을 더 넣으면 고를 수 있는 사람도 저절로 늘어난다. 사이드바에서 얼굴을 누르면
  누가 맡을지 고를 수 있고, 배정은 세션에 남아 다시 켜도 같은 얼굴이 같은 칸에 붙는다.
  `ui/assets/faces/<slug>.png` 를 넣으면 그때부터 그 그림이 붙고, 없으면 그 사람 색으로 칠한
  원이 대신한다. 일하는 중이면 pane 헤더의 얼굴 테두리가 숨을 쉰다
- **pane 이름** — 헤더를 두 번 눌러 붙인다. 세션에 남고, claude pane 이면 대화 이름도 같이 바뀐다
...
- **캐릭터 그림** — 자리와 배정은 다 됐고 정지 그림만 넣으면 된다. 20명 묘사는
  `ui/assets/faces/PROMPTS.md` 에 그림 생성에 넣을 수 있게 정리돼 있고, 움직임은
  `scripts/make-motion.py` 가 그 한 장에서 구워 준다
