...
- **캐릭터 얼굴** — 세션 목록의 동그란 아이콘과 pane 헤더의 점이 그 자리다. pane 마다
  로스터에서 한 명씩 배정해 얼굴을 붙이고, **일하는 중이면 그 얼굴이 반응하게** 하는 것이
  이 앱이 그리는 그림의 끝이다. 배정은 세션에 남아 다시 켜도 같은 얼굴이 같은 칸에 붙는다.
  막는 것은 스프라이트가 없다는 것 하나 — `theme-src-chiikawa/` 에는 외형 묘사와 로스터만
  있고 그림이 없다
